---
title: Business Development Manager – AI Services
excerpt: Bouw mee aan de toekomst van AI. Van strategie tot implementatie.
image: news/images/job_1.jpg
---

# Business Development Manager – AI Services

Bouw mee aan de toekomst van AI. Van strategie tot implementatie. Wij zijn een snelgroeiende AI scale-up met één missie: organisaties écht slimmer maken met AI. Geen buzzwords, maar werkende oplossingen die impact maken.

Daarom zoeken we een Business Development Manager die niet alleen deals sluit, maar ook inhoudelijk begrijpt wat we bouwen. Iemand die energie krijgt van nieuwe klanten binnenhalen, maar net zo enthousiast wordt van een goed model, een slimme architectuur of een geslaagde implementatie.


## Wat ga je doen?

### Nieuwe business creëren

- Jij opent deuren bij middelgrote en grote organisaties.
- Je bouwt je eigen pipeline en sluit zelfstandig deals.
- Je spot kansen in de markt en vertaalt die naar concrete AI-proposities.
- Je zit aan tafel met directie, innovatie- en datateams.

### Inhoudelijk het verschil maken

- Je denkt mee over AI-strategie, use cases en technische oplossingen.
- Je helpt bij presales, workshops en proof-of-concepts.
- Je kunt – waar nodig – zelf meewerken aan een prototype of technische validatie.
- Je slaat de brug tussen business en tech.

### Van deal naar duurzame relatie

- Je blijft betrokken na de handtekening.
- Je bewaakt waardecreatie en klanttevredenheid.
- Je ziet en realiseert upsell- en groeikansen.

## Wie ben jij?

- 5+ jaar ervaring in business development, tech consultancy of AI/IT-sales.
- Je begrijpt AI écht (machine learning, data pipelines, LLM’s, automatisering).
- Je kunt praten over modellen, maar ook over ROI.
- Je bent ondernemend, zelfstandig en comfortabel met targets.
- Je krijgt energie van bouwen — aan klanten én aan een organisatie.
- Je communiceert scherp en overtuigend (NL & EN).

Bonuspunten als je:

- Zelf hebt geprogrammeerd of data science ervaring hebt.
- Eerder in een startup of scale-up hebt gewerkt.
- Een sterk netwerk hebt in een specifieke sector.

## Wat bieden wij?

- Een uitdagende rol in een snelgroeiend team.
- De kans om met de nieuwste technologieën te werken.
- Een competitief salarispakket en flexibele werkomstandigheden.
